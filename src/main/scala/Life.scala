  ...
  process(clk.rising) {
    ...
    state match
      ...
      case NEIGH => {
        ...
        neigh_cnt :== (top_sr(0, 0).uint +^ top_sr(1, 1).uint +^
          top_sr(2, 2).uint +^ mid_sr(0, 0).uint +^ mid_sr(2, 2).uint +^
          bot_sr(0, 0).uint +^ bot_sr(1, 1).uint +^ bot_sr(2, 2).uint).bits
          .resize(neigh_cnt.width)
          .uint
